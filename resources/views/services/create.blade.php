...
@section('content')
<div class="card">
    <div class="card-header">Tạo dịch vụ mới</div>
    <div class="card-body">
        @if ($errors->any())
        ...
        <form action="{{ route('admin.services.store') }}" method="POST" class="row g-3">
            ...
            <div class="col-md-6">
                ...
                <input type="text" name="name" class="form-control @error('name') is-invalid @enderror" 
                       placeholder="Ví dụ: Cắt tóc nam" value="{{ old('name') }}" required>
                @error('name')
            ...
            <div class="col-md-6">
                ...
                <input type="number" name="price" class="form-control @error('price') is-invalid @enderror" 
                       placeholder="Ví dụ: 50000" value="{{ old('price') }}" step="1000" min="0" required>
                @error('price')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="col-md-6">
                ...
                <input type="number" name="duration_minutes" class="form-control @error('duration_minutes') is-invalid @enderror" 
                       placeholder="Ví dụ: 30" value="{{ old('duration_minutes') }}" min="1" required>
            ...
            <div class="col-12">
                ...
                <textarea name="description" class="form-control @error('description') is-invalid @enderror" 
                          rows="4" placeholder="Nhập mô tả dịch vụ">{{ old('description') }}</textarea>
                @error('description')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
